...
our %method_authentication = (
        'run_rast_workflow' => 'required',
        'rast_annotate_ama' => 'required',
);
...
sub call_method {
    ...
    my $ctx = kb_RASTServerContext->new($self->{loggers}->{userlog},
...
{
    # Service kb_RAST requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for kb_RAST but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = kb_RASTServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service kb_RAST.");
    }
...
package kb_RASTServerContext;

use strict;

=head1 NAME

kb_RASTServerContext
...
package kb_RASTServerStderrWrapper;
...
unless (caller) {
    ...
    {
        use kb_RASTImpl;
        my $obj = kb_RASTImpl->new;
        push(@dispatch, 'kb_RAST' => $obj);
    }
    my %headers = (
        "Authorization" => $token,
        "CLI" => "1"
    );
    my $server = kb_RASTServer->new(
